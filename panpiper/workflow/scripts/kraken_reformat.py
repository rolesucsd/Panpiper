...
def reformat_kraken(basename, krakenpath):
    ...
    df = pd.read_csv(filename, sep='\t', header=None, names=["D1","Contig","ID","Length","D2"])
    ...
    # Aggregate by the second column, concatenate all the contig IDs together in a list that is new column called contigs (also need to concatenate the length)
    # df = df.groupby('ID').apply(agg_df)
