...
python main.py -o ../../Package_test -a ../../Package_test/Quality/Assembly_filter -s  ../../Package_test/Quality/sample_list.txt -r /panfs/roles/Package_test/reference/9343.fna -w pangenome

#--cluster_type slurm --cluster_config cluster.json --cluster_args "sbatch -A {cluster.account} --mem {cluster.mem} -t {cluster.time} --cpus-per-task {cluster.cpus}"